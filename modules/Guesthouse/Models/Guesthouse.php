...
class Guesthouse extends Bookable
{
    ...
    static public function getSeoMetaForPageList()
    {
        ...
        if (!empty($title = setting_item_with_lang("guesthouse_page_list_seo_title", false))) {
            $meta['seo_title'] = $title;
        } else if (!empty($title = setting_item_with_lang("guesthouse_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("guesthouse_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        } else if (!empty($title = setting_item("guesthouse_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("guesthouse_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("guesthouse_page_list_seo_share");
        $meta['full_url'] = url(config('guesthouse.guesthouse_route_prefix'));
        return $meta;
    }

    public function terms()
    {
        return $this->hasMany($this->guesthouseTermClass, "target_id");
    }

    public function termsByAttributeInListingPage()
    {
        $attribute = setting_item("guesthouse_attribute_show_in_listing_page", 0);
        return $this->hasManyThrough($this->termClass, $this->guesthouseTermClass, 'target_id', 'id', 'id', 'term_id')->where('bravo_terms.attr_id', $attribute)->with(['translations']);
    }

    public function getAttributeInListingPage()
    {
        $attribute_id = setting_item("guesthouse_attribute_show_in_listing_page", 0);
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('guesthouse.guesthouse_route_prefix') . "/" . $this->slug;
        if (!empty($param)) {
    ...
    public static function getLinkForPageSearch($locale = false, $param = [])
    {
        ...
        return url(app_get_locale(false, false, '/') . config('guesthouse.guesthouse_route_prefix') . "?" . http_build_query($param));
    }
    ...
    {
        return url(route('guesthouse.admin.edit', ['id' => $this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($check) {
            ...
            if (!empty($this->tmp_selected_rooms)) {
                foreach ($this->tmp_selected_rooms as $room) {
                    if (isset($this->tmp_rooms_by_id[$room['id']])) {
                        $guesthouseRoomBooking = new GuesthouseRoomBooking();
                        $guesthouseRoomBooking->fillByAttr([
                            'room_id',
                            'parent_id',
                            'start_date',
                            'end_date',
    ...
    public function isAvailableInRanges($start_date, $end_date)
    {
        ...
        if ($this->default_state) {
            $notAvailableDates = $this->guesthouseDateClass::query()->where([
                [
                    'start_date',
                    '>=',
                    $start_date
            ...
            $availableDates = $this->guesthouseDateClass::query()->where([
                [
                    'start_date',
                    '>=',
                    $start_date
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('guesthouse_booking_buyer_fees');
        if (!empty($list_fees)) {
    ...
    {
        return setting_item("guesthouse_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("guesthouse_review_approved", 0);
    }

    public function check_enable_review_after_booking()
    {
        $option = setting_item("guesthouse_enable_review_after_booking", 0);
        if ($option) {
    ...
    {
        $options = setting_item("guesthouse_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("guesthouse_review_stats", []))) {
     ...
     * Get Score Review
     *
     * Using for loop guesthouse
     */
    public function getScoreReview()
    {
        $guesthouse_id = $this->id;
        $list_score = Cache::rememberForever('review_' . $this->type . '_' . $guesthouse_id, function () use ($guesthouse_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $guesthouse_id)->where('object_model', "guesthouse")->where("status", "approved")->first();
    ...
    public function saveCloneByID($clone_id)
    {
        ...
        //Language
        $langs = $this->guesthouseTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {
    ...
    {
        return setting_item('guesthouse_disable') == false;
    }

    public function isDepositEnable(){
        return (setting_item('guesthouse_deposit_enable') and setting_item('guesthouse_deposit_amount'));
    }
    public function getDepositAmount(){
        return setting_item('guesthouse_deposit_amount');
    }
    public function getDepositType(){
        return setting_item('guesthouse_deposit_type');
    }
    public function getDepositFomular(){
        return setting_item('guesthouse_deposit_fomular','default');
    }


    public function detailBookingEachDate($booking){
	    $rooms = \Modules\Guesthouse\Models\GuesthouseRoomBooking::getByBookingId($booking->id);
	    $roomsIds = $rooms->pluck('room_id')->toArray();
	    $roomsNumber = $rooms->pluck('number','room_id')->toArray();
	    $startDate = $booking->start_date;
	    $endDate = $booking->end_date;
	    $query = GuesthouseRoomDate::query();
    ...
    public static function isEnableEnquiry(){
        if(!empty(setting_item('booking_enquiry_for_guesthouse'))){
            return true;
        }
        return false;
    }
    ...
    public static function isFormEnquiryAndBook(){
        $check = setting_item('booking_enquiry_for_guesthouse');
    ...
    {
        $model_guesthouse = parent::query()->select("bravo_guesthouses.*");
        ...
        if (!empty($star_rate = $request->query('star_rate'))) {
            $model_guesthouse->WhereIn('star_rate',$star_rate);
        }
        ...
        if (is_array($terms) && !empty($terms)) {
            $model_guesthouse->join('bravo_guesthouse_term as tt', 'tt.target_id', "bravo_guesthouses.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_guesthouse->leftJoin('bravo_guesthouse_translations', function ($join) {
                    $join->on('bravo_guesthouses.id', '=', 'bravo_guesthouse_translations.origin_id');
                });
                $model_guesthouse->where('bravo_guesthouse_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_guesthouse->where('bravo_guesthouses.title', 'LIKE', '%' . $service_name . '%');
            }
        }
