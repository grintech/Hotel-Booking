...
@section('content')
    <h2 class="title-bar no-border-bottom">
        {{$row->id ? __('Edit: ').$row->title : __('Add new guesthouse')}}
        @if($row->id)
            <div class="title-action">
                <a class="btn btn-info" href="{{route('guesthouse.vendor.room.index',['guesthouse_id'=>$row->id])}}">
                    <i class="fa fa-hand-o-right"></i> {{__("Manage Rooms")}}
                </a>
                <a href="{{route('guesthouse.vendor.room.availability.index',['guesthouse_id'=>$row->id])}}" class="btn btn-warning">
    ...
    <div class="lang-content-box">
        <form action="{{route('guesthouse.vendor.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
            @csrf
