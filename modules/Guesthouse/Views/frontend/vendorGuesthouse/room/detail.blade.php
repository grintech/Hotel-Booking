...
@section('content')
    <h2 class="title-bar no-border-bottom">
        ...
        <div class="title-action">
            <a class="btn btn-info" href="{{route('guesthouse.vendor.room.index',['guesthouse_id'=>$guesthouse->id])}}">
    ...
    <div class="lang-content-box">
        <form novalidate action="{{route('guesthouse.vendor.room.store',['guesthouse_id'=>$guesthouse->id,'id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" class="needs-validation" method="post">
            @csrf
            <div class="form-add-service">
                ...
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-tour-content">
                        @include('Guesthouse::admin.room.form-detail.content')
                    </div>
                    @if(is_default_lang())
                        <div class="tab-pane fade" id="nav-tour-pricing">
                            @include('Guesthouse::admin.room.form-detail.pricing')
                        </div>
                        <div class="tab-pane fade" id="nav-attribute">
                            @include('Guesthouse::admin.room.form-detail.attributes')
                        </div>
                        <div class="tab-pane fade" id="nav-ical">
                            @include('Guesthouse::admin.room.form-detail.ical')
                        </div>
                    @endif
                </div>
            </div>
