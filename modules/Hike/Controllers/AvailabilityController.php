...
use Modules\Hike\Models\HikeDate;
...
class AvailabilityController extends FrontendController
{
    ...
    protected $indexView = 'Hike::frontend.user.availability';
    ...
    public function index(Request $request)
    {
        ...
        $page_title = __('Hikes Availability');
    ...
    public function loadDates(Request $request)
    {
        ...
        $hike = $this->hikeClass::find($request->query('id'));
        if (empty($hike)) {
            return $this->sendError(__('Hike not found'));
        }
        $is_single = $request->query('for_single');
        $query = $this->hikeDateClass::query();
...
//        for ($i = strtotime($request->query('start')); $i <= strtotime($request->query('end')); $i += DAY_IN_SECONDS) {
        ...
        foreach ($period as $dt){
            ...
            $date = [
                'id'         => rand(0, 999),
                'active'     => 0,
                'price'      => (!empty($hike->sale_price) and $hike->sale_price > 0 and $hike->sale_price < $hike->price) ? $hike->sale_price : $hike->price,
                'is_default' => true,
                'textColor'  => '#2791fe'
            ];
            if (!$is_single) {
            ...
            if ($request->input('for_single')) {
                if (empty(!$hike->max_people) and $hike->max_people < 1) {
                    $date['active'] = 0;
                }
            }
            if (!empty($hike->meta->enable_person_types) and $hike->meta->enable_person_types == 1) {
                $date['person_types'] = $hike->meta->person_types;
            ...
            // Open Hours
            if (!empty($hike->meta->enable_open_hours) and $hike->meta->enable_open_hours == 1) {
                $open_hours = $hike->meta->open_hours;
                $nDate = date('N', $i);
        ...
        if (!empty($rows)) {
            foreach ($rows as $row) {
                ...
                if (!empty($hike->meta->enable_person_types) and $hike->meta->enable_person_types == 1) {
                    $list_person_types = $hike->meta->person_types;
        ...
        $bookings = $this->bookingClass::getBookingInRanges($hike->id, $hike->type, $request->query('start'), $request->query('end'));
        if (!empty($bookings)) {
...
//                for ($i = strtotime($booking->start_date); $i <= strtotime($booking->end_date); $i += DAY_IN_SECONDS) {
        ...
        if (!empty($hike->ical_import_url)) {
            $startDate = $request->query('start');
            $endDate = $request->query('end');
            $timezone = setting_item('site_timezone', config('app.timezone'));
            try {
                $icalevents = new Ical($hike->ical_import_url, [
    ...
    public function store(Request $request)
    {
        ...
        $hike = $this->hikeClass::find($request->input('target_id'));
        $target_id = $request->input('target_id');
        if (empty($hike)) {
            return $this->sendError(__('Hike not found'));
        }
        if (!$this->hasPermission('hike_manage_others')) {
            if ($hike->create_user != Auth::id()) {
...
//        for ($i = strtotime($request->input('start_date')); $i <= strtotime($request->input('end_date')); $i += DAY_IN_SECONDS) {
        ...
        foreach ($period as $dt){
            $date = $this->hikeDateClass::where('start_date', $dt->format('Y-m-d'))->where('target_id', $target_id)->first();
            if (empty($date)) {
                $date = new $this->hikeDateClass();
